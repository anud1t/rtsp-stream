...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3 || argc % 2 != 1) {
        g_printerr("Usage: %s [port] [mount_point pipeline_description]...\n", argv[0]);
        g_printerr("Example: %s 8554 /cam1 \"( v4l2src device=/dev/video0 ! ... )\" /video1 \"( filesrc location=video.mp4 ! ... )\"\n", argv[0]);
        g_printerr("Example: %s 8555 /cam1 \"( v4l2src device=/dev/video0 ! ... )\"\n", argv[0]);
        return -1;
    }

    // Check if first argument is a port number
    if (argc >= 3) {
        gchar *first_arg = argv[1];
        gboolean is_port = TRUE;
        
        // Check if it's a number
        for (i = 0; first_arg[i] != '\0'; i++) {
            if (!g_ascii_isdigit(first_arg[i])) {
                is_port = FALSE;
                break;
            }
        }
